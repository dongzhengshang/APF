 ...
 * @date 2017/5/31
 */
public class TimeAxisView extends TextureView implements TextureView.SurfaceTextureListener {
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                Canvas ca = lockCanvas();
                drawScale(ca, getWidth(), getHeight());
                unlockCanvasAndPost(ca);
            }
        }).start();
    }
...
//                break;
    ...
    private void drawTriangle(Canvas canvas, int wight, int height) {
        ...
        drawTriangle(canvas, wight, height);
    }
