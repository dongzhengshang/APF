...
public class HttpUtils {
    ...
    public enum RequestType {
        JSON(KEY_CONTENT_TYPE_JSON), FORM(KEY_CONTENT_TYPE_X_WWW), UPLOAD(KEY_CONTENT_TYPE_UPLOAD_DATA);
            ...
            try {
                ...
                if (method != HttpRequestMethod.GET) {
                    ...
                    if (netEntity.getRequestType() == RequestType.FORM) {
                        ...
                        addFilesContent(netEntity.getUploadFiles() == null ? new UploadFile[0] : netEntity.getUploadFiles(), dataOutputStream);
                    }
    ...
    private static void addFilesContent(UploadFile[] files, DataOutputStream output) throws IOException {
        if (files != null) {
            for (UploadFile file : files) {
                ...
                split.append(KEY_CONTENT_DISPOSITION_HEAD).append(file.getFormName()).append("\";")
